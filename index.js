...
function switchSearchMethod(i) {
    if (g_last_search_method != i) {
        ...
        const type_btn = document.querySelector('[data-key="dialog-globalsearch"] #searchSyntaxCheck')
...
function changeGroupBy(i){               // i = 0 默认分组，i = 1 按文档分组
    const searchMore = document.querySelector('[data-key="dialog-globalsearch"] #searchMore')
...
class SimpleSearch extends siyuan.Plugin {
    ...
    inputSearchEvent(cfg) {
        mylog('搜索事件触发', cfg)
        // 需考虑搜索页签和搜索面板同时打开的情况，优先选择搜索面板的搜索框
        const searchInput = document.querySelector('[data-key="dialog-globalsearch"] #searchInput')
                         || document.querySelector('#layouts #searchInput')
        const simpleSearchInput = document.querySelector('[data-key="dialog-globalsearch"] #simpleSearchInput')
    ...
    loadedProtyleStaticEvent() {    
        ...
        if (g_highlight_keywords) { // 判断是否需要高亮关键词
            // 需考虑搜索页签和搜索面板同时打开的情况，优先选择搜索面板的搜索框
            const search_list = document.querySelector('[data-key="dialog-globalsearch"] #searchList')
            ...
            const search_preview = document.querySelector('[data-key="dialog-globalsearch"] #searchPreview')
    ...
    onLayoutReady() {
        ...
        const operationsAfterOpenSearch = function () {
            ...
            let originalSearchInput = // 需考虑搜索页签和搜索面板同时打开的情况，优先选择搜索面板的搜索框
                document.querySelector('[data-key="dialog-globalsearch"] #searchInput')
                || document.querySelector('#layouts #searchInput');
            // 新搜索框
            ...
            for (let i = 0; i < mutationsList.length; i++) {
                ...
                if (mutationsList[i].addedNodes.length) {
                    if (mutationsList[i].addedNodes[0].getAttribute('data-key') == "dialog-globalsearch") {// 判断全局搜索
                        ...
                        document.querySelector(`[data-key="dialog-globalsearch"] #searchOpen`).onclick = function () { // 确保按下在页签打开时搜索关键词不变
        ...
        console.log("simple search Plus start...")
    }
    ...
    onunload() {
        ...
        document.querySelectorAll('#simpleSearchInput').forEach(node => node.remove);
        console.log("simple search Plus stop...")
    }
};
